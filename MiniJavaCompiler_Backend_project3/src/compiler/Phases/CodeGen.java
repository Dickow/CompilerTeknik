...
public class CodeGen extends IRElementVisitorWithArgument<CODE> {
	...
	public CODE visitStatement(MJIf e, CODE code) throws VisitorException {
		...
		LC3label ifEndLabel = new LC3label();
		code.comment(" IF ");
		code.comment("Condition");
		visitExpression(e.getCondition(), code);

		code.pop(CODE.TMP0);
		code.add(new LC3BRZ(ifEndLabel));
		// if true.
	...
	public CODE visitStatement(MJIfElse e, CODE code) throws VisitorException {
		LC3label ifElseLabel = new LC3label();
		LC3label ifEndLabel = new LC3label();
		code.comment(" IF/ELSE ");
		code.comment("condition");
		// the if block will be visited if the condition evaluates true
		visitExpression(e.getCondition(), code);
		// do the branching if it was false = 0;
		code.pop(CODE.TMP0);
	...
	public CODE visitStatement(MJWhile e, CODE code) throws VisitorException {
		LC3label whileBlockLabel = new LC3label();
		LC3label whileEndLabel = new LC3label();
		code.comment(" WHILE BEGIN ");
		// beginning of the whileLoop
		code.add(whileBlockLabel);
		visitExpression(e.getCondition(), code);
		// pop the value to see if it is true
		code.pop(CODE.TMP0);
		...
		if (t.isClass()) {
			if (t.getName().equals("String")) {
				...
				throw new CodeGenException(
						"Cannot print other classes than String.");
			}
		} else if (t.isInt()) {
	...
	private void handleMethodCall(MJMethodCallExpr methodCallExpr, CODE code)
			throws VisitorException {

		// Labels for the dynamic link, Return address and Return value
		LC3label blab = code.newLabel();
		LC3label dlab = code.newLabel();
		LC3label methodlab = code.newLabel();
		code.add(new LC3BR(blab));
		code.commentline(" slot for SP ");
		code.add(dlab);
		code.add(new LC3int(0)); // put 0 into the return value of the admin
									// area
		...
		code.add(new LC3ST(CODE.SP, dlab)); // saves the return address of the
											// method call
		code.commentline(" save SFP ");
		code.add(new LC3STR(CODE.SFP, CODE.SP, 1)); // saves the dynamic link R6
													// in the stack
		code.commentline(" increment SP to save arguments and make space for admin area ");
		code.add(new LC3ADD(CODE.SP, CODE.SP, 3)); // moves the SP to after the
													// admin area

		// Arguments are passed from caller to callee
		...
		if (methodCallExpr.getTarget().isStatic()) {
			code.add(new LC3LD(CODE.TMP0, methodlab));
		} else {
			...
			code.add(new LC3LDR(CODE.TMP0, CODE.TMP0, methodCallExpr
					.getTarget().getSlot()));
		}

		// Returning after the methodcall is done
	...
	public CODE visitExpression(MJNegate e, CODE code) throws VisitorException {
		...
		LC3label trueValue = new LC3label();
		LC3label negateEndLabel = new LC3label();
		visitExpression(e.getArgument(), code);
	...
	@Override
	public CODE visitExpression(MJBoolean e, CODE code) throws VisitorException {
	...
	@Override
	public CODE visitExpression(MJInteger e, CODE code) throws VisitorException {
		LC3regs r = null;
	...
	public CODE visitExpression(MJIdentifierClass e, CODE code, boolean lvalue)
		...
		if (e instanceof MJSelector)
			return visitExpression((MJSelector) e, code, lvalue);
		if (e instanceof MJArray)
			return visitExpression((MJArray) e, code, lvalue);
		if (e instanceof MJIdentifier)
			return visitExpression((MJIdentifier) e, code, lvalue);
	...
	public CODE visitExpression(MJIdentifier e, CODE code, boolean lvalue)
		...
		if (lvalue) {
			...
			code.add(new LC3LDR(CODE.TMP0, CODE.SFP, 3 + e.getDeclaration()
					.getOffset()));
			code.push(CODE.TMP0);
		}
	...
	public CODE visitExpression(MJArray e, CODE code, boolean lvalue)
			throws VisitorException {
		LC3label sizelab = code.newLabel();
		LC3label contlab = code.newLabel();
		LC3label errorlab = code.newLabel();
	...
	private int assignOffsets(int offset, LinkedList<MJVariable> fieldList)
	...
	private void addHelperFunctions(CODE code) throws CodeGenException {
		...
		try {
			...
			System.err
					.println("Could not find class String. This cannot happen.");
			System.exit(-1);
		}
	...
	private void computeClassSizes(MJProgram e, CODE code)
			throws CodeGenException {
	...
	private void assignMethodOffsetsHelper(MJClass c, HashSet<MJClass> visited) {
		...
		if (!c.isTop()) {
			...
			methodTable = (HashMap<Integer, MJMethod>) sc.getMethodTable()
					.clone();
		...
		for (MJMethod m : c.getMethodList()) {
			...
			// Check whether a super class defines a method with the same
			// parameter types
			...
			for (MJMethod sm : methodTable.values()) {
				if (!sm.getName().equals(methodName))
					continue;
				if (sm.getParameters().size() != m.getParameters().size())
					continue;

				LinkedList<MJVariable> methodParameters = m.getParameters();
				LinkedList<MJVariable> supermethodParameters = sm
						.getParameters();
	...
	private void computeClassSizeHelper(MJClass c, HashSet<MJClass> visited)
			throws CodeGenException {
	...
	private void addRuntimeSetup(MJProgram e, CODE code)
			throws CodeGenException {
